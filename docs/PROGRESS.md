...
- 开机自启动：使用 `tauri-plugin-autostart` 插件，通过 Windows 注册表管理
- 修复保存设置时热键重复注册错误

- 全局快捷键：使用 `tauri-plugin-global-shortcut` 实现系统级热键，窗口失焦后仍可查词
- 弹窗居中显示：弹窗以鼠标位置为中心显示
- 弹窗关闭：Rust 层轮询检测前台窗口变化，失焦自动关闭
- 移除了鼠标划词功能（不稳定）
...
| 历史查询 | ✅ | 独立页面，查看历史查询记录，显示释义，点击可再次查询 |
| 运行日志 | ✅ | 记录查词、词本操作等日志，内部滚动 |
| 左侧导航栏 | ✅ | 可折叠/展开，标题水平居中，图标+文字导航 |
| 查询记录释义 | ✅ | 保存单词时截取前 5 条释义，列表显示最多 3 条 |
| 查询记录持久化 | ✅ | 查询记录保存到 localStorage，重启后数据不丢失 |
| 弹窗释义增强 | ✅ | 显示词形变化、双语例句、网络释义、同反义词 |
| 词形还原 | ✅ | 查询时自动识别词形变化，自动匹配大小写 |
| 添加词本重复检查 | ✅ | 添加单词到词本时检查是否已存在，重复显示提示 |
| 开机自启动 | ✅ | `tauri-plugin-autostart`，通过注册表管理，设置页面开关 |
...
| `register_hotkey` | 注册全局快捷键 |
| `unregister_all_hotkeys` | 注销全部快捷键 |
