...
返回 `Settings` 对象，包含 `maimemoToken`、`hotkey`、`selectedNotepadId`、`autoStart` 字段。
...
// result.data.web_translations → string[]
...
await invoke('show_popup_window', {
  ...
  voc_id: 'voc_id...',
  token: '...',
  word_forms: [{ form: '复数', value: 'hellos' }],
  web_translations: ['喂', '你好'],
...
| examples | object[] | 例句 `{ sentence, translation }` |
| phonetic | string | 美式音标 |
| uk_phonetic | string | 英式音标 |
| voc_id | string | 墨墨词汇 ID（可为空） |
| token | string | 墨墨 API Token |
| word_forms | object[] | 词形变化 `{ form, value }` |
| web_translations | string[] | 网络释义 |
| synonyms | string[] | 同义词 |
| antonyms | string[] | 反义词 |

窗口特性：无边框、置顶、不显示在任务栏、不可调整大小、失焦自动关闭。
...
获取鼠标光标屏幕坐标（仅 Windows，其他平台返回默认值）。
