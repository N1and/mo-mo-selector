...
- 新增开机自启动功能，使用 `tauri-plugin-autostart` 插件
- 设置页面新增"开机自启动"开关
- 通过 Windows 注册表 `HKCU\...\Run` 管理自启动
- App 启动时自动同步自启动状态
...
- 修复保存设置时"HotKey already registered"错误
- 热键注册统一由 WordLookup 组件管理，Settings 保存时仅注销旧热键
- `unregister_all_hotkeys` 忽略无热键注册时的错误
...
- 热键改为系统级全局快捷键（`tauri-plugin-global-shortcut`）
- 窗口失焦后仍可用 `Ctrl+Shift+A` 触发查词
- 支持自定义快捷键，保存后即时生效
- 快捷键变更时自动注销旧热键并注册新热键
...
- 改为 Rust 层轮询 `GetForegroundWindow` HWND 对比检测失焦
- 弹窗显示后保持 2 秒，之后每 2 秒检查是否失焦

#### Token 检查优化

- `lookupWord` 改为动态从 store 读取 token，不再受初始加载时序影响

### 修复

#### 快捷键双击问题

- 移除 setup 中重复的热键注册，改为 WordLookup 挂载时统一注册
- 前端增加 1 秒防抖，避免快速连续触发
