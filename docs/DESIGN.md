...
1. 用户在任何应用中先复制英文单词到剪贴板
2. 按全局快捷键（默认 `Ctrl+Shift+A`，窗口失焦也有效）
3. Rust 全局快捷键插件捕获热键，派发 DOM 事件到前端
4. 前端读取剪贴板内容，验证是否为英文单词
5. 并行请求墨墨 API（获取 `voc_id`）和有道词典 API（获取释义）
6. Rust 后端创建独立弹窗窗口，以鼠标光标为中心显示
7. 通过 URL 参数传递单词数据给弹窗
8. 弹窗显示单词、音标、释义、例句
9. 用户可点击"加入学习"或"添加到词本"
10. 弹窗失焦或按 Escape 自动关闭（Rust 层轮询检测前台窗口变化）
11. 查询记录保存到本地，显示在历史查询页面
